    ...
    <ul class="navbar-nav mr-auto">
      ...
      <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu" aria-labelledby="navbarDropdown">
		  ...
		  <a class="dropdown-item active" href="usecode.php"><i class="fas fa-exchange-alt"></i> สะสมแต้ม</a>
...
<center>
			<div class="card" style="width: 95%;border: 2px solid black;">
			  <div class="card-header text-white text-left bg-dark" style="font-size: 20px;"><i class="fas fa-exchange-alt"></i> สะสมแต้ม</div>
			  <div class="card-body" style="max-height: 350px;overflow: scroll;"><form ACTION="<?php echo $usecodeFormAction; ?>"  id="paywallet" name="paywallet" method="POST" enctype="multipart/form-paycard">
			  <img src="assets/img/Code11_barcode.png" width="300px"><div style="padding-top:1%;"></div>
				<div class="input-group mb-3 w-75" style="opacity: 0.7;">
				  <div class="input-group-prepend">
					<span class="input-group-text" id="basic-addon3">แลกแต้มไปที่</span>
				  </div>
				  <input type="text" class="form-control text-center" name="imySID" value="<?php echo $row_Recordset1['SID']; ?>" disabled><input type="hidden" name="mySID" value="<?php echo $row_Recordset1['SID']; ?>">
				</div>
				<div class="input-group mb-3 w-75">
				  <div class="input-group-prepend">
					<span class="input-group-text" id="basic-addon3">กรอกReward Code</span>
				  </div>
				  <input type="text" class="form-control" name="code" required>
				  <div class="input-group-append">
					<button class="btn btn-dark" type="submit" id="button-addon2" >ตรวจสอบReward Code</button>
				  </div><input type="hidden" name="MM_insert" value="paywallet">
				</div></form>
				</div></div>
...
?>
<?php
// *** Validate request to code to this site.
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "paywallet")) {
$usecodeFormAction = $_SERVER['PHP_SELF'];
if (isset($_SERVER['QUERY_STRING'])) {
  $usecodeFormAction .= "?UseCode" . htmlentities($_SERVER['QUERY_STRING']);
}
	$uaa = $_POST['code'];
	$check = "SELECT * FROM rewardcode_data WHERE code = '$uaa'";
	$result2 = mysql_query($check) or die(mysql_error());
	$num=mysql_num_rows($result2);
    if($num > 0)   			
        {		
	$LoginRS__query=sprintf("SELECT reward FROM rewardcode_data WHERE code=%s",
  ...
  $updateSQL = sprintf("UPDATE user_profile SET reward_point=%s WHERE SID=%s ",
		GetSQLValueString($row_Recordset1['reward_point'] + $loginFoundUser['reward'], "text"),
        GetSQLValueString($_POST['mySID'], "text"));
 	
  mysql_select_db($database_myconnect, $myconnect);
  $Result1 = mysql_query($updateSQL, $myconnect) or die(mysql_error());

	$delSQL = sprintf("DELETE FROM rewardcode_data WHERE code=%s",
						GetSQLValueString($_POST['code'], "text"));
	
	  mysql_select_db($database_myconnect, $myconnect);
	 $Result1 = mysql_query($delSQL, $myconnect) or die(mysql_error());
    echo "<script type=\"text/javascript\">";
	echo "swal({title: \"ใช้งาน Reward Codeสำเร็จ\",
            text: \"คุณได้รับแต้มจำนวน ", $loginFoundUser['reward'] , " แต้มสำเร็จ...\",
            icon: \"success\",
            buttons: false,
          })\n";
	echo "setTimeout(function(){ window.location.href=\"./usecode.php\"; }, 2500);";
	echo "</script>";
 
		}else{
		
	echo "<script type=\"text/javascript\">";
	echo "swal({title: \"ไม่มี Reward Code นี้ในระบบ\",
