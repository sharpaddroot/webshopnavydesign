    ...
    <ul class="navbar-nav mr-auto">
      ...
      <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu" aria-labelledby="navbarDropdown">
          ...
          <a class="dropdown-item  active" href="sentstar.php"><i class="fas fa-gift"></i> โอนแต้มให้เพื่อน</a>
...
<center>
			<div class="card" style="width: 95%;border: 2px solid black;">
			  <div class="card-header text-white text-left bg-info" style="font-size: 20px;"><i class="fas fa-gift"></i> โอนแต้มให้เพื่อน</div>
			  ...
			  <img src="assets/img/Code11_barcode.png" width="300px"><div style="padding-top:1%;"></div>
				<div class="input-group mb-3 w-75">
				  <div class="input-group-prepend">
					<span class="input-group-text" id="basic-addon3">SIDของเพื่อน</span>
				  </div>
				  <input type="text" class="form-control text-center" name="mySID" required>
				</div>
				<div class="input-group mb-3 w-75">
				  <div class="input-group-prepend">
					<span class="input-group-text" id="basic-addon3">กรอกจำนวนแต้มที่จะโอน</span>
				  </div>
				  <input type="number" class="form-control" name="number_payment" required>
...
if (isset($_SERVER['QUERY_STRING'])) {
  $addpointFormAction .= "?InsertProduct" . htmlentities($_SERVER['QUERY_STRING']);
	...
	echo "swal({title: \"ต้องโอนแต้มทีละมากกว่า 0 แต้มครับ\",
            text: \"ระบบกำลังพาท่านไป...\",
            icon: \"error\",
            buttons: false,
          })\n";
	echo "setTimeout(function(){ window.location.href=\"sentstar.php\"; }, 2500);";
	echo "</script>";
	}
    else{
	if ($_POST['number_payment'] > $row_Recordset1['reward_point']) {
	echo "<script type=\"text/javascript\">";
	echo "swal({title: \"แต้มไม่พอทำการโอนครับ\",
            text: \"ระบบกำลังพาท่านไป...\",
            icon: \"error\",
            buttons: false,
          })\n";
  ...
  $updateSQL = sprintf("UPDATE user_profile SET reward_point=%s WHERE SID=%s ",
		GetSQLValueString($row_Recordset1['reward_point'] - $_POST['number_payment'], "int"),
        GetSQLValueString($row_Recordset1['SID'], "text"));
   mysql_select_db($database_myconnect, $myconnect);
  $Result1 = mysql_query($updateSQL, $myconnect) or die(mysql_error());

  $updateSQL = sprintf("UPDATE user_profile SET reward_point=%s WHERE SID=%s ",
		GetSQLValueString($_POST['number_payment'] + $row_Recordset2['reward_point'], "int"),
  ...
  $Result1 = mysql_query($updateSQL, $myconnect) or die(mysql_error());
    ...
    echo "<script type=\"text/javascript\">";
	echo "swal({title: \"โอนแต้มจำนวน ", $_POST['number_payment'] , " แต้มสำเร็จ\",
            text: \"โอนไปให้ ",$_POST['mySID'],"\",
            icon: \"success\",
            buttons: false,
          })\n";
	echo "setTimeout(function(){ window.location.href=\"./sentstar.php\"; }, 2500);";
