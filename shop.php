...
<form ACTION="<?php echo $addpointFormAction; ?>" id="paywallet" name="paywallet" method="POST" enctype="multipart/form-paycard">
					  ...
					  <div class="card-body">
						<h5 class="card-title">Item : <?php echo $row_product_data['product_name'] ?></h5>
						<p class="card-text">Item Type : <?php echo $row_product_data['product_type'] ?></p>
						<p class="card-text">Item Class : <?php echo $row_product_data['product_class'] ?></p>
						<p class="card-text">Detail : <?php echo $row_product_data['product_detail'] ?></p>
						<input type="text" style="display: none;" name="productid" value="<?php echo $row_product_data['product_id'] ?>"/>
						<button  type="submit" class="btn btn-success w-100">Price <?php echo $row_product_data['product_price'] ?> Points</button>
...
if (isset($_SERVER['QUERY_STRING'])) {
  $addpointFormAction .= "?BuyProduct" . htmlentities($_SERVER['QUERY_STRING']);
}	
		mysql_select_db($database_myconnect, $myconnect);
		$query_Recordset2 = sprintf("SELECT * FROM product_data WHERE product_id = %s", GetSQLValueString($_POST['productid'], "text"));
		$Recordset2 = mysql_query($query_Recordset2, $myconnect) or die(mysql_error());
		$row_Recordset2 = mysql_fetch_assoc($Recordset2);
		$totalRows_Recordset2 = mysql_num_rows($Recordset2);	
	
	if ($row_Recordset2['product_price'] > $row_Recordset1['pay_point']) {
	...
	echo "setTimeout(function(){ window.location.href=\"shop.php\"; }, 2500);";
	echo "</script>";
	}
    else{
  ...
  $updateSQL = sprintf("UPDATE user_profile SET pay_point=%s WHERE SID=%s ",
		GetSQLValueString($row_Recordset1['pay_point'] - $row_Recordset2['product_price'], "int"),
        GetSQLValueString($row_Recordset1['SID'], "text"));
   mysql_select_db($database_myconnect, $myconnect);
  $Result1 = mysql_query($updateSQL, $myconnect) or die(mysql_error());
    date_default_timezone_set("Asia/Bangkok");
	$insertSQL = sprintf("INSERT INTO log_productbuy (log_productname, log_price, log_name, log_date, buy_number)  VALUES (%s,%s,%s,%s,%s) ",
					   GetSQLValueString($row_Recordset2['product_name'], "text"),
					   GetSQLValueString($row_Recordset2['product_price'], "text"),
    ...
    echo "<script type=\"text/javascript\">";
	echo "swal({title: \"คุณทำการซื้อ ", $row_Recordset2['product_name'] , " สำเร็จ\",
