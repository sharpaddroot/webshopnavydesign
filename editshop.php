...
<center>
			<div class="card" style="width: 95%;border: 2px solid black;">
			  <div class="card-header text-white text-left bg-danger" style="font-size: 20px;"><i class="fas fa-shopping-cart"></i> ร้านค้าไอเทม</div>
...
<form ACTION="<?php echo $insertproimgFormAction; ?>" id="form78" name="form78" method="POST" enctype="multipart/form-data">
					  <img class="card-img-top" src="./assets/img/product_img/unproduct.jpg" alt="Card image cap">
					  <div class="card-body">
						...
						<p class="card-text"><input type="text" name="class" class="form-control" placeholder="Item Class" required></p>
...
<form ACTION="<?php echo $updateproimgFormAction; ?>" id="form79" name="form79" method="POST" enctype="multipart/form-data">
					  <img class="card-img-top" src="./assets/img/product_img/<?php echo $row_product_data['product_img'] ?>" alt="Card image cap">
					  <div class="card-body">
						<h5 class="card-title"><input type="text" name="name" class="form-control" value="<?php echo $row_product_data['product_name'] ?>"></h5>
						<p class="card-text"><input type="text" name="type" class="form-control" value="<?php echo $row_product_data['product_type'] ?>" class="form-control" value="<?php echo $row_product_data['product_type'] ?>"></p>
						<p class="card-text"><input type="text" name="class" class="form-control" value="<?php echo $row_product_data['product_class'] ?>" class="form-control" value="<?php echo $row_product_data['product_class'] ?>"></p>
						<p class="card-text"><input type="text" name="detail" class="form-control" value="<?php echo $row_product_data['product_detail'] ?>" class="form-control" value="<?php echo $row_product_data['product_detail'] ?>"></p>
						<p class="card-text"><input type="number" name="price" class="form-control" value="<?php echo $row_product_data['product_price'] ?>" class="form-control" value="<?php echo $row_product_data['product_price'] ?>"></p>
						<input type="text" style="display: none;" name="proimgid" value="<?php echo $row_product_data['product_id'] ?>"/>
...
<input type="hidden" name="name_proimg" value="<?php echo 'product_imgId='.$row_product_data['product_id'] ?>">
</form>
						<div style="padding-top:4%;"></div>
	<form ACTION="<?php echo $delproimgFormAction; ?>" id="form80" name="form80" method="POST" enctype="multipart/form-data">
		<input type="text" style="display: none;" name="proimgid1" value="<?php echo $row_product_data['product_id'] ?>"/>
						<button  type="submit" class="btn btn-danger w-100">ลบสินค้า</button>
	<input type="hidden" name="MM_insert" value="form80">
</form>
					  </div>
					</div>
				</div>
<?php } while($row_product_data = mysql_fetch_array($product_data)); ?>
...
<?php mysql_free_result($product_data) ?>
<?php
